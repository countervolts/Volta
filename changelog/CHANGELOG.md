...
- [x] fixed Plex (and tightened every backend) so playback serves the original file by default and only transcodes when you actually ask for it: Plex used to silently fall back to a 320 kbps transcode whenever a track's file-part key wasn't already cached, and Subsonic never sent `format=raw` so the server could transcode to its own default. Added a shared `StreamingPreferences.wantsTranscode` rule (transcode only when a bitrate cap or target format is set), made Plex warm + cache the part key before building the stream URL (`prepareForPlayback`/`streamMetadataReady`, with the player pre-warming the current and next couple of tracks so gapless/AutoMix preloads stay original too), made Subsonic force `format=raw` for originals, and made Jellyfin honor an explicit format choice even at original bitrate
